    ...
    public function cardsIncidents()
    {
        ...
        if (isset($this->request->start) && isset($this->request->end)) {
            foreach($indicadores as $indicador) {
                ...
                $cantidadDiaInicioToDiaActualAnterior[] = Incident::with('Indicator')->where('indicator', $indicador)->whereBetween('date_ipat', [$hace_30_dias->copy()->subDays($diasTranscurridos), $hoy->copy()->subDays($diasTranscurridos)])->count();

                $cantidadDiaInicioToDiaActualActual[] = Incident::with('Indicator')->where('indicator', $indicador)->whereBetween('date_ipat', [$hace_30_dias, $hoy])->count();
            }
        } else {
            foreach($indicadores as $indicador) {

                $cantidadDiaInicioToDiaActualAnterior[] = Incident::with('Indicator')->where('indicator', $indicador)->whereBetween('date_ipat', [$hace_30_dias->copy()->subDays($diasTranscurridos), $hace_30_dias->copy()->subDays(0)])->count();

                $cantidadDiaInicioToDiaActualActual[] = Incident::with('Indicator')->where('indicator', $indicador)->whereBetween('date_ipat', [$primerDiaDelMes, $hoy])->count();
            }
        }
