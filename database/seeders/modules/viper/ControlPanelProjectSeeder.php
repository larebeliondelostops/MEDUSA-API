...
class ControlPanelProjectSeeder extends Seeder
{
    /**
    ...
    public function createControlPanelProjectForProject($projectId)
    {
        ...
        foreach ($namesAndStageControl as $item) {
            $now = Carbon::now();
            $stageControlId = DB::table('stage_control')
            ->where('name', $item['stage_control'])
            ->value('id');
            $controlPanelId = DB::table('control_panel')
                    ->where('stage_control_id', $stageControlId)
            ...
            DB::table('control_panel_project')->insert($controlPanelProject);
        }
    }
}
